 ...
 * Grab the lock -> check exit condition -> fetch a task -> release the lock -> encoding -> write result -> up semaphore
 */
void *thread_controller(){

    while(1){
        sem_wait(thread_pool->remain_task);      // if there are tasks to do, otherwise, block and wait for tasks

        pthread_mutex_lock(thread_pool->task_queue_lock);
        pthread_mutex_lock(thread_pool->task_submission_finished_lock);
        if (thread_pool->task_submission_finished == 1){    // if no more tasks
            pthread_mutex_unlock(thread_pool->task_submission_finished_lock);
            if (!(thread_pool->task_head->next)){    // if empty task queue
                thread_pool->task_tail = thread_pool->task_head;
                sem_post(thread_pool->remain_task);
        ...
        unsigned char *result;
        task_queue *tmp = thread_pool->task_head->next;
        thread_pool->task_head->next = tmp->next;
        ...
        *(thread_pool->result + tmp->task_id) = result;
        sem_post(thread_pool->result_sem + tmp->task_id);
        tmp->next = NULL;
        free(tmp);
    }
}
...
void *collect_result(){
    ...
    for (long i = 0; ; i++){
        ...
        if (thread_pool->task_submission_finished == 1) {    // if no more tasks
            pthread_mutex_unlock(thread_pool->task_submission_finished_lock);
            pthread_mutex_lock(thread_pool->task_count_lock);
            if (i == thread_pool->task_count) {      // if read all results
                pthread_mutex_unlock(thread_pool->task_count_lock);
                sem_post(thread_pool->all_task_finished);
                sem_post(thread_pool->remain_task);
                printf("%s", reserved);
                pthread_exit(NULL);     // exit
            }
        ...
        sem_wait(thread_pool->result_sem + i);
        result = *(thread_pool->result + i);
        res_len = strlen((char*)result) - 2;
        if (reserved[0] == result[0]){
            result[1] += reserved[1];
        }
